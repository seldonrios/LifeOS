...
function extractMemoryType(eventType) {
...
function eventToSearchQuery(event) {
...
export function createDeterministicEmbedding(text) {
    const vector = new Array(EMBEDDING_DIMENSION).fill(0);
...
async function loadTransformersExtractor(modelName) {
...
function extractEmbeddingArray(output) {
    if (output &&
        ...
        return output.data.map((value) => (Number.isFinite(value) ? value : 0));
    }
    if (Array.isArray(output)) {
        const flattened = output.flat(3).filter((value) => typeof value === 'number');
        return flattened;
    }
    if (output && typeof output === 'object' && 'tolist' in output) {
        const tolist = output.tolist;
        if (typeof tolist === 'function') {
            const listed = tolist();
            if (Array.isArray(listed)) {
                return listed.flat(3).filter((value) => typeof value === 'number');
            }
        }
    }
    return [];
...
export class TransformersEmbeddingProvider {
    ...
    async embed(text) {
        try {
            const extractor = (await loadTransformersExtractor(this.modelName));
...
export class MemoryManager {
    ...
    async addToThread(threadId, input) {
    ...
    async getRelevantContext(eventOrQuery, options = {}) {
        const query = typeof eventOrQuery === 'string' ? eventOrQuery.trim() : eventToSearchQuery(eventOrQuery);
        if (!query) {
            return [];
        }
    ...
    async getRelevantContextForCurrentConversation(query, options = {}) {
        ...
        const threadContext = options.threadId && options.threadId.trim().length > 0
    ...
    async getRelevantContextForToday(limit = this.contextLimit) {
    ...
    findLastSummaryIndex(thread, threadId) {
    ...
    async summarizeThreadIfNeeded(threadId) {
        try {
