        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a practical research assistant. Summarize in 2-3 concise sentences. No markdown.',
          },
          {
            role: 'user',
...
async function handleResearchQuery(
  ...
  try {
    const generated = await summarizeResearch(query, context, fetchFn);
  ...
  const saved = await client.appendResearchResult({
    query,
    summary,
    sources: [source],
    savedAt: nowIso,
  ...
  await context.publish(
    Topics.lifeos.researchCompleted,
    {
      id: saved.id,
      query: saved.query,
  ...
  return {
    ...
    async init(context: ModuleRuntimeContext): Promise<void> {
      await context.subscribe<Record<string, unknown>>(
        ...
        async (event) => {
          const query = resolveResearchQuery(toVoicePayload(event));
          if (!query) {
          ...
          try {
            await handleResearchQuery(query, context, fetchFn, now);
          } catch (error: unknown) {
      ...
      await context.subscribe<AgentWorkPayload>(Topics.agent.workRequested, async (event) => {
        ...
        if (!query) {
          context.log('[Research] Agent payload missing query.');
          return;
        }
