...
import { HouseholdAutomationFailedSchema, HouseholdChoreCreateRequestedSchema, Topics, } from '@lifeos/module-sdk';
...
export async function createChoreIntentStore(dbPath) {
    ...
    applyBestEffortMigration(db, 'CREATE UNIQUE INDEX IF NOT EXISTS idx_chores_original_capture_id ON chores(original_capture_id) WHERE original_capture_id IS NOT NULL');
    return {
        createRequestedChore(payload) {
            const existing = db
                .prepare(`SELECT id, title, assigned_to_user_id, due_at
           FROM chores
         ...
         VALUES (?, ?, ?, ?, ?, 'pending', NULL, NULL, NULL, ?, ?)`).run(choreId, payload.householdId, payload.choreTitle, payload.actorUserId, dueAt, createdAt, payload.originalCaptureId);
            return {
                householdId: payload.householdId,
                choreId,
...
function createModuleObservabilityClient(context) {
...
export function resolveChoreAutomationFailure(error, input) {
    ...
    if (/unsupported recurrence frequency|unable to compute next allowed byday occurrence|recurrence/i.test(message)) {
        return {
...
async function publishAutomationFailure(eventBus, payload) {
    const event = {
        id: randomUUID(),
...
export function createHouseholdChoresModule(options = {}) {
    ...
    return {
        ...
        async init(context) {
            ...
            if (!dbPath) {
                context.log('[household-chores] skipped intent subscription: missing LIFEOS_HOUSEHOLD_DB_PATH');
                return;
            }
            const store = await createIntentStore(dbPath);
            const observabilityClient = options.observabilityClient ?? createModuleObservabilityClient(context);
            await context.subscribe(Topics.lifeos.householdChoreCreateRequested, async (event) => {
                ...
                try {
                    ...
                    if (failure) {
                        ...
                        await publishAutomationFailure(context.eventBus, HouseholdAutomationFailedSchema.parse({
...
export const householdChoresModule = createHouseholdChoresModule();
export async function publishChoreAssigned(context, payload) {
    await context.publish(Topics.lifeos.householdChoreAssigned, payload, 'dashboard-service');
}
export async function publishChoreCompleted(context, payload) {
