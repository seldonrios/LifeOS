...
import { HouseholdHomeStateChangedSchema, HouseholdVoiceCaptureCreatedSchema, HomeNodeStateSnapshotUpdatedSchema, } from '@lifeos/contracts';
import { buildNextSnapshot } from '@lifeos/home-node-core/dist/client.js';
...
async function publishSnapshotUpdated(context, householdId, snapshot) {
...
export const homeStateModule = {
    ...
    async init(context) {
        ...
        await context.subscribe(Topics.lifeos.householdHomeStateChanged, async (event) => {
            ...
            context.log(`[home-state] published snapshot update for ${payload.householdId} from ${payload.stateKey}`);
        });
