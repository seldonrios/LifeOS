...
test('createHousehold returns a persisted row with UUID id and name', () => {
    ...
    try {
        ...
        assert.match(household.id, /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i);
...
test('acceptInvite rejects invalid invite expiry timestamps', () => {
    ...
    try {
        ...
        const token = generateInviteToken();
        assert.throws(() => client.storeInviteToken(household.id, 'user-4', token, 'not-a-valid-datetime'), /valid ISO datetime/i);
    }
    finally {
        cleanup();
    }
...
test('createReminder persists sensitive flag with default and explicit values', () => {
    ...
    try {
        const household = client.createHousehold('Home Base');
        const defaultReminder = client.createReminder(household.id, 'custom', 'object-1', ['user-1'], '2026-04-01T10:00:00.000Z');
        const sensitiveReminder = client.createReminder(household.id, 'custom', 'object-2', ['user-2'], '2026-04-02T10:00:00.000Z', true);
...
test('clearPurchasedItems archives purchased rows and leaves active rows untouched', () => {
    ...
    try {
        ...
        const listId = client.getOrCreateDefaultShoppingListId(household.id);
        ...
        rawDb
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(purchasedId, listId, household.id, 'Milk', 'admin-1', 'admin-1', 'purchased', 'manual', new Date().toISOString());
        rawDb
            .prepare(`INSERT INTO shopping_items
          (id, list_id, household_id, title, added_by, added_by_user_id, status, source, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(activeId, listId, household.id, 'Eggs', 'admin-1', 'admin-1', 'added', 'manual', new Date().toISOString());
...
test('evaluateReminderAutomationFailures returns no-token fix suggestion', () => {
    const { client, cleanup } = createTestClient();
    try {
        const household = client.createHousehold('Reminder Home', JSON.stringify({
            notificationRouting: {
                members: {
                    'adult-1': {
                        pushToken: '',
        ...
        const token = generateInviteToken();
        client.storeInviteToken(household.id, 'adult-1', token, new Date(Date.now() + 60_000).toISOString());
        client.acceptInvite(token, household.id);
        const failures = client.evaluateReminderAutomationFailures(household.id, ['adult-1'], '2026-03-31T12:00:00.000Z');
...
test('evaluateReminderAutomationFailures returns no-token failure when member profile is missing', () => {
    const { client, cleanup } = createTestClient();
    try {
        const household = client.createHousehold('Missing Profile Home', JSON.stringify({
            notificationRouting: {
                members: {},
            },
        }));
        ...
        const token = generateInviteToken();
        client.storeInviteToken(household.id, 'adult-2', token, new Date(Date.now() + 60_000).toISOString());
        client.acceptInvite(token, household.id);
        const failures = client.evaluateReminderAutomationFailures(household.id, ['adult-2'], '2026-03-31T12:00:00.000Z');
        assert.equal(failures.length, 1);
        assert.equal(failures[0]?.errorCode, 'REMINDER_NO_TOKEN');
...
test('evaluateReminderAutomationFailures returns quiet-hours suppression', () => {
    const { client, cleanup } = createTestClient();
    try {
        const household = client.createHousehold('Quiet Home', JSON.stringify({
            timeZone: 'UTC',
            notificationRouting: {
                members: {
                    'adult-1': {
        ...
        client.storeInviteToken(household.id, 'adult-1', token, new Date(Date.now() + 60_000).toISOString());
        client.acceptInvite(token, household.id);
        const failures = client.evaluateReminderAutomationFailures(household.id, ['adult-1'], '2026-03-31T23:30:00.000Z');
        assert.equal(failures.length, 1);
...
test('evaluateReminderAutomationFailures returns inactive-device failure', () => {
    const { client, cleanup } = createTestClient();
    try {
        const household = client.createHousehold('Inactive Home', JSON.stringify({
            notificationRouting: {
                members: {
                    'adult-1': {
                        pushToken: 'expo-token-1',
...
test('isWithinQuietHours — America/New_York timezone, server in UTC', () => {
    ...
    try {
        ...
        const household = client.createHousehold('TZ Home NY', JSON.stringify({
        ...
        const token = generateInviteToken();
        client.storeInviteToken(household.id, 'user-tz-1', token, new Date(Date.now() + 60_000).toISOString());
        client.acceptInvite(token, household.id);
        // 2026-04-12T04:00:00.000Z = 00:00 EDT (UTC-4) — inside quiet 22:00–07:00
        const failures = client.evaluateReminderAutomationFailures(household.id, ['user-tz-1'], '2026-04-12T04:00:00.000Z');
        assert.equal(failures.length, 1);
        assert.equal(failures[0]?.errorCode, 'REMINDER_QUIET_HOURS');
        assert.equal(failures[0]?.deliveryStatus, 'quiet_hours_suppressed');
    }
...
test('isWithinQuietHours — cross-midnight range in non-UTC timezone (America/Chicago)', () => {
    const { client, cleanup } = createTestClient();
    try {
        const household = client.createHousehold('TZ Home Chicago', JSON.stringify({
        ...
        const token = generateInviteToken();
        client.storeInviteToken(household.id, 'user-tz-2', token, new Date(Date.now() + 60_000).toISOString());
        client.acceptInvite(token, household.id);
        // 2026-04-12T05:00:00.000Z = 00:00 CDT (UTC-5) — inside quiet 22:00–06:00
        const failuresInside = client.evaluateReminderAutomationFailures(household.id, ['user-tz-2'], '2026-04-12T05:00:00.000Z');
        ...
        const failuresOutside = client.evaluateReminderAutomationFailures(household.id, ['user-tz-2'], '2026-04-12T14:00:00.000Z');
...
test('isWithinQuietHours — DST transition hour does not throw', () => {
    const { client, cleanup } = createTestClient();
    try {
        const household = client.createHousehold('TZ DST Home', JSON.stringify({
            timeZone: 'America/New_York',
            notificationRouting: {
                members: {
                    'user-dst': {
        ...
        const token = generateInviteToken();
        client.storeInviteToken(household.id, 'user-dst', token, new Date(Date.now() + 60_000).toISOString());
        client.acceptInvite(token, household.id);
        // 2026-03-08T07:00:00.000Z — DST spring-forward transition in America/New_York
        let threw = false;
        let result;
        try {
            result = client.evaluateReminderAutomationFailures(household.id, ['user-dst'], '2026-03-08T07:00:00.000Z');
        }
        catch {
            threw = true;
        }
...
test('isWithinQuietHours — no timeZone falls back to server local time and emits warning', () => {
    ...
    try {
        const household = client.createHousehold('No TZ Home', JSON.stringify({
            notificationRouting: {
                members: {
                    'user-notz': {
        ...
        const token = generateInviteToken();
        client.storeInviteToken(household.id, 'user-notz', token, new Date(Date.now() + 60_000).toISOString());
        client.acceptInvite(token, household.id);
        let threw = false;
        let result;
        try {
            result = client.evaluateReminderAutomationFailures(household.id, ['user-notz'], '2026-04-12T12:00:00.000Z');
        }
        catch {
            threw = true;
        }
        ...
        assert.equal(payload.message, 'isWithinQuietHours: no timeZone configured, falling back to server local time');
...
test('listChores — 50 chores with 2 active members returns all with correct assignedTo', () => {
    ...
    try {
        ...
        client.storeInviteToken(household.id, 'member-a', tokenA, new Date(Date.now() + 60_000).toISOString());
        client.storeInviteToken(household.id, 'member-b', tokenB, new Date(Date.now() + 60_000).toISOString());
        client.acceptInvite(tokenA, household.id);
        client.acceptInvite(tokenB, household.id);
        for (let i = 0; i < 50; i++) {
            client.createChore(household.id, `Chore ${i}`, i % 2 === 0 ? 'member-a' : 'member-b', new Date(Date.now() + 86_400_000).toISOString());
        }
        ...
        for (const chore of chores) {
            const expectedUser = chore.title.endsWith('0') ||
                ...
                ? 'member-a'
                : 'member-b';
            assert.ok(chore.assignedTo.userId === 'member-a' || chore.assignedTo.userId === 'member-b', `Expected member-a or member-b, got ${chore.assignedTo.userId}`);
...
test('listChores — identical results with known assignment data', () => {
    ...
    try {
        ...
        const token = generateInviteToken();
        client.storeInviteToken(household.id, 'user-known', token, new Date(Date.now() + 60_000).toISOString());
        client.acceptInvite(token, household.id);
        const chore = client.createChore(household.id, 'Wash Dishes', 'user-known', new Date(Date.now() + 86_400_000).toISOString());
        const chores = client.listChores(household.id);
